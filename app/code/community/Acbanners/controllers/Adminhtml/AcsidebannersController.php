...
class AffinityCloud_Acbanners_Adminhtml_AcsidebannersController extends Mage_Adminhtml_Controller_Action {
    ...
    public function indexAction() {
        $this->_initAction();       
        $this->_addContent($this->getLayout()->createBlock('acbanners/adminhtml_acsidebanners'));
        $this->renderLayout();
    }
    ...
    public function editAction()
    {
        ...
        $bannersModel = Mage::getModel('acbanners/acsidebanners')->load($bannersId);
        ...
        if ($bannersModel->getId() || $bannersId == 0) {
            ...
            $this->_addContent($this->getLayout()->createBlock('acbanners/adminhtml_acsidebanners_edit'))->_addLeft($this->getLayout()->createBlock('acbanners/adminhtml_acsidebanners_edit_tabs'));
    ...
    public function saveAction()
    {
        ...
        if ( ! empty($_FILES['image']['name'])) {
            try {
                ...
                $path = Mage::getBaseDir('media').DS.'acsidebanners';

                $uploader->save($path, $fileName);

                $imageData['image'] = 'acsidebanners/'.$fileName;
            } catch (Exception $e) {
                return $this->error($e);
            }
        }
    ...
    public function deleteAction()
    {
        if($this->getRequest()->getParam('id') > 0) {
            try {
                $bannersModel = Mage::getModel('acbanners/acsidebanners');
    ...
    {
        $this->loadLayout();
        $this->getResponse()->setBody($this->getLayout()->createBlock('acbanners/adminhtml_acsidebanners_grid')->toHtml());
    }
